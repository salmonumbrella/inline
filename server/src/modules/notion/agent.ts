...
async function createNotionPage(input: { spaceId: number; chatId: number; messageId: number; currentUserId: number }) {
  ...
  const [notionUsers, database, samplePages, targetMessage] = await Promise.all([
  ...
  const messages = await MessageModel.getMessagesAroundTarget(input.chatId, input.messageId, 15, 15)
...
Instructions
	...
	•	User Assignment Rules:
		...
		▪	Reporter/Watcher: Set to the user that matches with target message sender. (who sent the message/report the thing) the user that we find from notion user form this is ${
      targetMessage.fromId
    }
...
${messages
  .map(
    (m: any, index: number) =>
      `[${index}] messageId: ${m.messageId} userId: ${m.fromId} Date: ${m.date || ""} Text: ${
        m.text || ""
...
//     - Subsequent messages: "Can you do this?" followed by "Sure, I'll have it ready by Monday"
//     - Related context: "The deadline is next week" mentioned earlier in conversation
//   - **User Matching**: Match chat participants with Notion users based on names, emails, or usernames.
//   - Use the current user id to find the user in the notion users list and set it as the assignee and the target message sender as the reporter/watcher.
//   - Set user that matched with current user id as asignee and creator of the task
//   - Generate ONLY the properties object that matches the provided schema structure - do not include parent or top-level fields

//   # Examples
//   Note: The examples below use generic property names. You must use the EXACT property names from the database schema provided.
...
//   Database has properties: "Name" (title), "Due Date" (date), "Status" (status), "Assignee" (people)
//   </example_context>
//   <assistant_response>
//   {
//     "Name": {
...
//   Database has properties: "Task" (title), "Deadline" (date), "State" (status)
//   </example_context>
//   <assistant_response>
//   {
//     "Task": {
...
//   Target Message Sender ID: ${targetMessage.fromId}
//   All Participants: ${JSON.stringify(participantNames, null, 2)}
//   </chat_participants>
//   `
// }
